...
class GestureDetector:
    ...
    def process_frame(self) -> Tuple[Optional[np.ndarray], str, float]:
        ...
        if landmarks:
            ...
            # Movement gestures (W,A,S,D) = FAST response, no stabilization
            movement_gestures = ["point_left", "point_right", "point_up", "closed_fist"]
            if raw_gesture in movement_gestures and raw_confidence > 0.7:
                self.current_gesture = raw_gesture
                self.gesture_confidence = raw_confidence
                display_gesture = raw_gesture
                display_confidence = raw_confidence
            else:
                # Action gestures (Space, Mouse) = STABLE response with stabilization
